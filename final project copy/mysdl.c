...
int main(int32_t argc,char *argv[]){
	...
	button_setting_rect(&option_button,screen_width*0.265,screen_height*0.015,screen_width*0.47,screen_height*0.076);
	//backpack
	...
	SDL_GetMouseState(&mouse_x,&mouse_y);
					...
					if(open_settings==1){
						...
						for(int32_t i=0;i<3;i++){
							...
							Mix_PlayChannel(-1,sound1,0);
								...
								nowtalk_line_which,sound_line_which,character_list,num_character,start_option,backpack,0);
								quit=1;
								break;
								}
							}
		...
		render_image_block(chatbar.u.s,&chatbar_block,0,0,230);
		}
		
		//顯示現在講話是誰
		if(strlen(now_talk_str)!=0){
		render_image_block(chatbar.u.s,&nowtalk_block,0,0,230);
		...
		renderStaticText(now_text_str,screen_width*0.12,screen_height*0.687,1,&event);
			...
			options_size=0;
				...
				//確認是不是隱藏選項(好感度)
				int32_t notshow=0;
				toml_array_t *hide_like_array=NULL;
				hide_like_array=toml_array_in(now_options,"hide_like");
				if(hide_like_array!=NULL){
				toml_datum_t hide_like_name = toml_string_at(hide_like_array,0);
				toml_datum_t hide_like_value = toml_int_at(hide_like_array,1);
					for(int32_t i=0;i<num_character;i++){
						if(strcmp(character_list[i].name,hide_like_name.u.s)==0){
				...
				toml_datum_t now_item_describe;
					...
					now_item_table = toml_table_in(item_table,backpack[i]);
						...
						if(item_select==i){
							...
							render_option_Button(backpack[i],&item_block,screen_width*0.682,
							screen_height*0.4);
				...
				if(open_backpack==1){
					for(int32_t i=0;i<9;i++){
						...
						(i/3)*screen_height*0.25)){
							...
							if(backpack_button_3.ok){
							//render_image_block(backpack_button_3.u.s,&item_bg_block,(i%3)*screen_width*0.21,
							//(i/3)*screen_height*0.25,255);
							}
						}
					}
				...
				if(open_save==1 || open_load==1){
					for(int32_t i=0;i<8;i++){
						if(check_button(mouse_x,mouse_y,&save_data_block,0,i*screen_height*0.08)){
							...
							if(save_button_3.ok){
							//render_image_block(save_button_3.u.s,&save_data_block,0,i*screen_height*0.08,255);
							}
						}
					}
				}
