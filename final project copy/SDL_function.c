...
int32_t check_save(struct Character *character_list,int32_t num_character,int32_t save_which){
	...
	char nowtalk_line_which_str[1000]={0};
	char sound_line_which_str[1000]={0};
	char start_option_str[1000]={0};
...
void render_end_text(const char *text){
	...
	//加載字體
	TTF_Font *endfont = TTF_OpenFont("Cubic_11_1.100_R.ttf",SCREEN_WIDTH*0.05);
	// 渲染按鈕文本
...
void render_saveinfo_Button(char now_event_name_str[1000],char now_scene_name_str[1000],char timestamp[1000],button_rect *button,int32_t offset_x,int32_t offset_y){
	...
	SDL_Texture *textTexture = SDL_CreateTextureFromSurface(rend, textSurface);
	SDL_Rect destRect = {button->x+SCREEN_WIDTH*0.03,button->y+(button->h-textSurface->h)/2+offset_y,textSurface->w,textSurface->h}; 
	// 渲染文字紋理
...
void render_discribe_Text(const char *text,button_rect *button,int32_t offset_x,int32_t offset_y){
	SDL_Color textColor = {255, 255, 255}; // 黑色文字顏色
...
void render_llm_Text(const char *text, int32_t x, int32_t y,SDL_Event *event){
	...
	SDL_RenderCopy(rend, textTexture, NULL, &destRect);
		...
		/*// 依次渲染文字
		SDL_Surface *textSurface = TTF_RenderText_Blended_Wrapped(font,text,textColor,500); 
		SDL_Texture *textTexture = SDL_CreateTextureFromSurface(rend, textSurface);
...
int32_t initialize(){
	...
	//加載字體
	font = TTF_OpenFont("Cubic_11_1.100_R.ttf",SCREEN_WIDTH*0.023);
	if(font==NULL){
	printf("無法加載字體 SDL: %s\n", TTF_GetError());
	return 0;
	}
