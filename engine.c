...
int main(int32_t argc,char *argv[]){
    ...
    character_key=toml_key_in(character_table,i);
    	...
    	toml_datum_t now_character_like = toml_int_in(now_character_table,"like");
    		...
    		strncpy(character_list[i].name,now_character_name.u.s, 200);
    		}
    		else{
    		printf("角色名稱有錯誤！\n");
    		return 0;
	...
	char now_event_music_str[200]={0};
	char now_talk_str[200]={0};
	char now_text_str[200]={0};
	char now_options_next_str[200]={0};
	//音效
	...
	//選項按鈕
	toml_datum_t options_button_1 = toml_string_in(game_default, "options_button_1");
	toml_datum_t options_button_2 = toml_string_in(game_default, "options_button_2");
	//開場按鈕
	...
	SDL_GetMouseState(&mouse_x,&mouse_y);
				...
				case SDL_MOUSEBUTTONDOWN:
					...
					if(start_open==1){
						for(int32_t i=0;i<4;i++){
							if(check_button(mouse_x,mouse_y,&open_option_block,0,i*screen_height*0.1+screen_height*0.1) && open_OpeningSettings==0 && open_load==0){
							//按鈕音效
							Mix_PlayChannel(-1,sound1,0);
								...
								//一開始事件都是event.start
								strncpy(now_event_str,"start", 200);
								}
								//載入(開啟)
								if(i==1){
								open_load=1;
					...
					if(open_OpeningSettings==1){
						...
						for(int32_t i=-2;i<3;i++){
							...
							//音效調整
				if(check_button(mouse_x,mouse_y,&open_option_block,screen_width*0.2*i,screen_height*-0.1+screen_height*0.2) && open_OpeningSettings==1){
								if(i==-2){
									if(sound1!=NULL){
					...
					if(open_settings==1){
						...
						for(int32_t i=-2;i<3;i++){
							...
							//音效調整
				if(check_button(mouse_x,mouse_y,&open_option_block,screen_width*0.2*i,screen_height*-0.1+screen_height*0.2) && open_settings==1){
								if(i==-2){
									if(sound1!=NULL){
									Mix_VolumeChunk(sound1,0);
									}
						...
						for(int32_t i=0;i<3;i++){
							if(check_button(mouse_x,mouse_y,&open_option_block,0,i*screen_height*0.1+screen_height*0.2)){
							//按鈕音效
							Mix_PlayChannel(-1,sound1,0);
								//設定(關閉)
								if(i==0){
		...
		if(start_open==1 && open_pause==0){
			if(play_open_music_check==0){
				...
				if(open_music!=NULL){
				Mix_VolumeChunk(open_music,MIX_MAX_VOLUME*music_rate);
				open_music_channel=Mix_PlayChannel(-1,open_music,-1);
				}
		...
		render_scene(open_bg.u.s);
			...
			for(int32_t i=0;i<4;i++){
			render_image_block(default_button_1.u.s,&open_option_block,0,i*screen_height*0.1+screen_height*0.1,255);	
			}
		render_option_Button("新遊戲",&open_option_block,0,0*screen_height*0.1+screen_height*0.1);
		render_option_Button("載入遊戲",&open_option_block,0,1*screen_height*0.1+screen_height*0.1);
		render_option_Button("遊戲設定",&open_option_block,0,2*screen_height*0.1+screen_height*0.1);
		render_option_Button("離開遊戲",&open_option_block,0,3*screen_height*0.1+screen_height*0.1);
		open_pause=1;
		//截圖
		screenshot();
		}
		...
		init_str(now_event_name_str);
		strncpy(now_event_name_str,now_event_name.u.s, 200);
		}
		...
		init_str(now_scene_name_str);
		strncpy(now_scene_name_str,now_scene_name.u.s, 200);
		}
		//讀入scene.background
		...
		toml_datum_t now_scene_bg = toml_string_in(now_scene_table, "background");
			...
			//如果名稱一樣就繼續播放
				...
				Mix_HaltChannel(music_channel);
				strncpy(now_event_music_str,now_event_music.u.s,200);
					// 加載音效文件
					music1=loadSound(now_event_music.u.s);
					if(music1!=NULL){
					// 播放音效 -1無限巡迴
					Mix_VolumeChunk(music1,MIX_MAX_VOLUME*music_rate);
			...
			//把現在background存下來
			init_str(now_scene_bg_str);
			strncpy(now_scene_bg_str,now_scene_bg.u.s, 200);
			//漸入
			...
			init_str(now_scene_bg_str);
				if(missing_image.ok){
				strncpy(now_scene_bg_str,missing_image.u.s,200);
				}
			//漸入
			fadein(now_scene_bg_str,&event);	
			}
		...
		toml_datum_t now_dialogue = toml_string_in(now_event_table, "dialogue");
			...
			strncpy(now_dialogue_str,now_dialogue.u.s, 200);
			}
			else{
			printf("現在事件沒有第一個對話\n");
			return 0;
			...
			if(key!=NULL){
				...
				if(dialogue_line.ok){
					...
					sscanf(key,"character%d",&key_ch_which);
						...
						init_str(now_character_image_str);
						init_str(now_character_avatar_str);
						strncpy(now_character_str,dialogue_line.u.s, 200);
						//對話換下一條
						dialogue_line_which++;
						//不是在文字
						text_catch=0;
					...
					sscanf(key,"emote%d",&key_emote_which);
						...
						emote_line_which++;
							...
							now_character_avatar = toml_string_in(now_character_table,now_avatar_str);
								...
								strncpy(now_character_image_str,now_character_image.u.s,200);
								}
								else{
									if(missing_image.ok){
									strncpy(now_character_image_str,missing_image.u.s,200);
									}
								}
								if(now_character_avatar.ok){
								strncpy(now_character_avatar_str,now_character_avatar.u.s,200);
								}
								else{
									if(missing_image.ok){
									strncpy(now_character_avatar_str,missing_image.u.s,200);
									}
								}
							//對話換下一條
							dialogue_line_which++;
					...
					sscanf(key,"nowtalk%d",&key_ch_which);
						...
						//把現在人物存下來
						init_str(now_talk_str);
						strncpy(now_talk_str,dialogue_line.u.s, 200);
						//對話換下一條
						dialogue_line_which++;
						//不是在文字
						text_catch=0;
					...
					sscanf(key,"sound%d",&key_sound_which);
						...
						dialogue_sound=loadSound(dialogue_line.u.s);
							...
							Mix_VolumeChunk(dialogue_sound,MIX_MAX_VOLUME*sound_rate);
					...
					sscanf(key,"text%d",&key_text_which);
						...
						//把現在文字抓下來
						init_str(now_text_str);
						strncpy(now_text_str,dialogue_line.u.s, 200);
						//是在文字
						text_catch=1;
						}
						else{
					...
					options_size=0;
					//抓下一個對話的名稱
					strncpy(now_dialogue_str,dialogue_line.u.s, 200);
					continue;
					}
					//抓下一個事件
					...
					options_size=0;
					//抓下一個事件的名稱
					strncpy(now_event_str,dialogue_line.u.s, 200);
					//漸黑特效
					...
					Mix_HaltChannel(music_channel);
					fadeout(now_scene_bg_str,&event);
					render_end_text(dialogue_line.u.s,&event);
					//回到標題
					start_game=0;
					start_open=1;
					open_pause=0;
			...
			options_size=0;
				...
				toml_datum_t now_options_event = toml_string_in(now_options, "event");
					if(option_select==options_size){
						...
						options_size=0;
						//抓下一個對話的名稱
						strncpy(now_dialogue_str,now_options_next.u.s, 200);
						break;
						}
						//如果是event
						...
						options_size=0;
						//抓下一個事件的名稱
						strncpy(now_event_str,now_options_event.u.s, 200);
						next_event=1;
						break;
						}
						else{
					...
					if(option_select==0){
					render_image_block(options_button_1.u.s,&option_button,0,options_size*screen_height*0.09,255);
		...
		for(int32_t i=0;i<options_size;i++){
			if(check_button(mouse_x,mouse_y,&option_button,0,(i+1)*screen_height*0.09) && start_game==1 && start_option==1 && press_hide==0 && press_CharacterDiscribe==0 && open_backpack==0 && open_save==0  && open_settings==0){
				if(options_button_2.ok){
				render_image_block(options_button_2.u.s,&option_button,0,(i+1)*screen_height*0.09,255);
				icon_Change=1;
				}
			}
		}
		...
		for(int32_t i=0;i<4;i++){
			if(check_button(mouse_x,mouse_y,&open_option_block,0,i*screen_height*0.1+screen_height*0.1) && start_open==1 && open_load==0 && open_OpeningSettings==0){
				if(default_button_2.ok){
				render_image_block(default_button_2.u.s,&open_option_block,0,i*screen_height*0.1+screen_height*0.1,255);
				icon_Change=1;
				}
			}
		}
		...
		for(int32_t i=0;i<3;i++){
			if(check_button(mouse_x,mouse_y,&open_option_block,0,i*screen_height*0.1+screen_height*0.2) && open_settings==1){
				if(default_button_2.ok){
				render_image_block(default_button_2.u.s,&open_option_block,0,i*screen_height*0.1+screen_height*0.2,255);
				icon_Change=1;
				}
			}
		}
		...
		for(int32_t i=-2;i<3;i++){
			if(check_button(mouse_x,mouse_y,&open_option_block,screen_width*0.2*i,screen_height*-0.1) && (open_settings==1 || open_OpeningSettings==1)){
				if(default_button_2.ok){
				render_image_block(default_button_2.u.s,&open_option_block,screen_width*0.2*i,screen_height*-0.1,255);	
				icon_Change=1;
				}
			}
			if(check_button(mouse_x,mouse_y,&open_option_block,screen_width*0.2*i,screen_height*-0.1+screen_height*0.2) && (open_settings==1 || open_OpeningSettings==1)){
				if(default_button_2.ok){
				render_image_block(default_button_2.u.s,&open_option_block,screen_width*0.2*i,
				screen_height*-0.1+screen_height*0.2,255);	
				icon_Change=1;
				}
			}
		}
			...
			darker("assets/image/screenshot.bmp");
			//調整音樂音量
			render_image_block(chatbar.u.s,&nowtalk_block,screen_width*-0.07,screen_height*-0.52,255);
			render_option_Button("音樂調整",&nowtalk_block,screen_width*-0.07,screen_height*-0.52);
				for(int32_t i=-2;i<3;i++){
				render_image_block(default_button_1.u.s,&open_option_block,screen_width*0.2*i,screen_height*-0.1,255);	
				}
			...
			//調整音效音量
			render_image_block(chatbar.u.s,&nowtalk_block,screen_width*-0.07,screen_height*-0.32,255);
			render_option_Button("音效調整",&nowtalk_block,screen_width*-0.07,screen_height*-0.32);
				for(int32_t i=-2;i<3;i++){
				render_image_block(default_button_1.u.s,&open_option_block,screen_width*0.2*i,screen_height*-0.1+screen_height*0.2,255);	
				}
			render_option_Button("0%",&open_option_block,2*screen_width*-0.2,screen_height*-0.1+screen_height*0.2);
			render_option_Button("25%",&open_option_block,1*screen_width*-0.2,screen_height*-0.1+screen_height*0.2);
			render_option_Button("50%",&open_option_block,0*screen_width*0.2,screen_height*-0.1+screen_height*0.2);
			render_option_Button("75%",&open_option_block,1*screen_width*0.2,screen_height*-0.1+screen_height*0.2);
			render_option_Button("100%",&open_option_block,2*screen_width*0.2,screen_height*-0.1+screen_height*0.2);
			
				for(int32_t i=0;i<3;i++){
				render_image_block(default_button_1.u.s,&open_option_block,0,i*screen_height*0.1+screen_height*0.2,255);	
				}
			render_option_Button("返回遊戲",&open_option_block,0,0*screen_height*0.1+screen_height*0.2);
			render_option_Button("回到標題",&open_option_block,0,1*screen_height*0.1+screen_height*0.2);
			render_option_Button("離開遊戲",&open_option_block,0,2*screen_height*0.1+screen_height*0.2);
			settings_pause=1;
			//截圖
			screenshot();
			}			
