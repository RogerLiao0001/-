...
int main(int32_t argc,char *argv[]){
	...
	SDL_GetMouseState(&mouse_x,&mouse_y);
		...
		render_image_block(open_title.u.s,&title_block,0,0,255);
			...
			for(int32_t i=0;i<4;i++){
			render_image_block(open_button_1.u.s,&open_option_block,0,i*screen_height*0.1+screen_height*0.1,255);	
			}
		...
		for(int32_t i=0;i<4;i++){
			if(check_button(mouse_x,mouse_y,&open_option_block,0,i*screen_height*0.1+screen_height*0.1) && start_open==1 && open_load==0 && open_OpeningSettings==0){
				if(open_button_2.ok){
				render_image_block(open_button_2.u.s,&open_option_block,0,i*screen_height*0.1+screen_height*0.1,255);
				icon_Change=1;
				}
			}
		}
